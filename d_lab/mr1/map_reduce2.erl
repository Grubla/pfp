...
map_reduce_dist(Map,M,Reduce,R,Input) ->
  ...
  Splits = split_into(M,Input),
  Mappers = 
      [spawn_mapper_dist(Parent,Map,R,Split,pool()) 
          || Split <- Splits],
  Mappeds = 
    [receive {Ref,L} -> L end || Ref <- Mappers],
  Reducers = [spawn_reducer_dist(Parent,Reduce,I,Mappeds,pool())
      || I <- lists:seq(0,R-1)],
  Reduceds = [receive {Ref,L} -> L end || Ref <- Reducers],
  lists:sort(lists:flatten(Reduceds)).

spawn_mapper_dist(Parent,Map,R,Split,Pool)->
  Ref = make_ref(),
  Pool ! {get_node, self()},
  receive {use_node,Node} ->
    spawn_link(Node, fun()->
      Mapped = [{erlang:phash2(K2,R),{K2,V2}} 
          || {K,V} <- Split, {K2,V2} <- Map(K,V)],
      SortedMapped = group(lists:sort(Mapped)),
      Pool ! {available,Node},
      Parent ! {Ref,SortedMapped} end) end,
  Ref.

spawn_reducer_dist(Parent,Reduce,I,Mappeds,Pool) ->
  Ref = make_ref(),
  Pool ! {get_node, self()},
  Inputs = [KV || Mapped <- Mappeds, {J,KVs} <- Mapped,I==J,KV <-KVs],
  receive {use_node,Node} ->
    spawn_link(Node, fun() -> 
      ReducedInputs = reduce_seq(Reduce,Inputs),
      Pool ! {available,Node},
      Parent ! {Ref,ReducedInputs} end) end,
  Ref.
...
pool([]) ->
  receive {available,Node} -> pool([Node]) end;
pool([Node|Nodes]) -> 
  receive {get_node,Pid} -> Pid ! {use_node,Node}, pool(Nodes) end.
...
spawn_mapper(Parent,Map,R,Split)->
  spawn_link(fun()->
      Mapped = [{erlang:phash2(K2,R),{K2,V2}} 
          || {K,V} <- Split, {K2,V2} <- Map(K,V)],
   Parent ! {self(),group(lists:sort(Mapped))} end).
