...
map_reduce_dist(Map,M,Reduce,R,Input) ->
  ...
  Mappers = 
      [spawn_mapper_dist(Parent,Map,R,Split) 
          || Split <- Splits],
  Mappeds = 
    [receive {Ref,L} -> L end || Pid <- Mappers],
...
spawn_mapper_dist(Parent,Map,R,Split)->
  Ref = make_ref(),
  spawn_link('yoo@ed3354-05', fun()->
      Mapped = [{erlang:phash2(K2,R),{K2,V2}} 
          || {K,V} <- Split, {K2,V2} <- Map(K,V)],
   Parent ! {Ref,group(lists:sort(Mapped))} end),
  Ref.

spawn_reducer_dist(Parent,Reduce,I,Mappeds) ->
  Ref = make_ref(),
  Inputs = [KV || Mapped <- Mappeds, {J,KVs} <- Mapped,I==J,KV <-KVs],
  spawn_link('yoo@ed3354-05', fun() -> Parent ! {Ref,reduce_seq(Reduce,Inputs)} end),
  Ref.
