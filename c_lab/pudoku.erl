...
pRefine(M) ->
    ...
    %RestM = droplast(M),
    spawn_link(fun() -> 
        Parent ! {Ref, refine_row(lists:last(M))} end),
    pRefine(droplast(M)) ++
        receive {Ref,Solved} -> [Solved] end.
...
%     [M].
droplast(M) ->
    lists:reverse(tl(lists:reverse(M))).
...
pSolve([P|Ps]) ->
    ...
    Ref = make_ref(), 
    spawn_link(fun() -> 
        Parent ! {Ref, Name, bm(fun()->solve(M) end)} end),
    pSolve(Ps) ++ 
        receive {Ref,Nam,Solved} -> [{Nam,Solved}] end.
pBenchmarks() ->
    {ok,Puzzles} = file:consult("problems.txt"),
    timer:tc(?MODULE,pSolve,[Puzzles]).
