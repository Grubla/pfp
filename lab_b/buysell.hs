...
-- The naive  sequential algorithm
...
-- The naive algorithm in parallel
...
getIDByMaxP bestID currID as
  ...
  | otherwise                         = getIDByMaxP bestID (currID+1) as
		where (Z:.n) = extent as
-- A better algorithm that does not scale as well in parallel
...
bestSell' as bestIndex currIndex
  | as!(Z:.bestIndex) >= as!(Z:.currIndex) = 
    computeS $ R.append (bestSell' as bestIndex (currIndex-1)) (arrIdx bestIndex)
  | otherwise                              = 
    computeS $ R.append (bestSell' as currIndex (currIndex-1)) (arrIdx currIndex)

arrIdx :: Int -> Array U DIM1 Int
arrIdx i = (fromListUnboxed (Z :. (1::Int)) [i] :: Array U DIM1 Int)

-- A sequential version of the better algorithm
mainMainGS :: [Int] -> Array U DIM0 (Int, Int, Int)
mainMainGS as = buySellGS $ (fromListUnboxed 
  (Z :. ((length as)::Int)) as :: Array U DIM1 Int)
...
bestSellS' as bestIndex currIndex
  | as!(Z:.bestIndex) >= as!(Z:.currIndex) =
    computeS $ R.append (bestSellS' as bestIndex (currIndex-1)) (arrIdxS bestIndex)
  | otherwise                              = 
    computeS $ R.append (bestSellS' as currIndex (currIndex-1)) (arrIdxS currIndex)
